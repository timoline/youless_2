...
<?php
	...
	if($ok)
	{
		...
		try {
		    $db = new PDO('mysql:host='.$config['db_host'].';dbname='.$config['db_name'], $config['db_user'], $config['db_pass'], array( 
      			PDO::MYSQL_ATTR_USE_BUFFERED_QUERY => true
   			));
		    $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); 
		
		    $succes = $db->exec("
				CREATE TABLE IF NOT EXISTS `".$config['db_name']."`. `kwh_h` (
				...
				INSERT INTO `".$config['db_name']."`. `settings` (`key`, `value`) VALUES
				('cpkwh_low', '0.10'),
				('dualcount', '0'),
				...
				CREATE TABLE IF NOT EXISTS `".$config['db_name']."`. `meters` (
				  `id` int(11) NOT NULL AUTO_INCREMENT,
				  `name` varchar(40) COLLATE utf8_bin NOT NULL,
				  `address` varchar(15) COLLATE utf8_bin NOT NULL,
